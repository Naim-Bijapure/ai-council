...
- **WHEN** the ChatGPT page contains a usable prompt input and send controls
...
The ChatGPT adapter SHALL submit the council prompt to ChatGPT through the page UI.

#### Scenario: Prompt is injected and sent
- **WHEN** the background sends a ChatGPT automation command with a valid prompt
- **THEN** the adapter writes the prompt into the ChatGPT input and triggers the send action
